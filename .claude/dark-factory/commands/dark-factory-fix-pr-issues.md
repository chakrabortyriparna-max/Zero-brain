...
5. **Respect project-specific hard invariants** (per CLAUDE.md). If the codebase has documented invariants (message caps, embedding models, streaming formats, auth flows, etc.), do not alter them unless the validator explicitly says to.
...
3. **Run the relevant local check immediately after each file change.** Detect the project type, then run the matching checker:

   **Python projects** (look for `pyproject.toml`, `setup.py`, `requirements.txt`):
   - `ruff check <path>` (or `flake8`, `pylint` if ruff is absent)
   - `mypy <path>` (if types are used)
   - `pytest tests/<file> -xvs` (or `unittest`)

   **JavaScript / TypeScript projects** (look for `package.json`):
   - `tsc --noEmit` (if TypeScript)
   - `eslint src/<path>` (or `biome check`, `prettier --check`)
   - `vitest run` / `jest` / `npm test` (if a test script exists)
...
Before committing, run the full local validation suite to make sure you haven't regressed anything. Detect the project type first, then run the matching suite.
...
mypy .                  # if used
pytest tests -xvs       # if tests directory exists
```

**JavaScript / TypeScript (look for `package.json`):**
```bash
tsc --noEmit            # if TypeScript
eslint src              # or biome check, prettier --check
npm test                # or vitest run, jest, pnpm test, bun test
```
...
- **LOCAL_VALIDATION_GREEN**: Lint and type-check pass on the changed files before you committed.

---
description: Holdout-pattern E2E validator. Drives agent-browser against the running application to verify that the PR's user-facing behavior actually matches what the linked issue asked for.
...
6. **Any source code files** — the source code is out of bounds. You drive the browser only.
...
Common generic flows to test:
- **Health endpoint**: verify the app responds on its base URL and any known health/status endpoints return 200.
- **Main functionality**: exercise the primary user-facing feature the PR touches (e.g., submit a form, trigger a workflow, view a dashboard).
- **Error handling**: intentionally trigger a failure (empty input, invalid URL, unauthorized action) and verify the UI shows a readable error instead of a crash or a silent failure.
...
- `flows_tested`: array of strings — names of user flows you exercised (e.g., `"health_endpoint"`, `"main_functionality"`, `"error_handling"`)

...
4. `behavioral-validation.scope_appropriate == "too_broad"` AND `unrequested_changes` is non-empty AND contains architecture-scale changes (swapped core framework, new auth system, new public API surface)
...
7. PR touches any hard invariants per CLAUDE.md (rate limit, auth, core config)

...
- SQL injection: string-concatenated SQL. Flag any SQL outside designated repository/query files or any concatenation. Parameterized queries are expected.
...
- Any new dependency entry in `pyproject.toml`, `package.json`, `requirements.txt`, `Cargo.toml`, or similar — flag with name, version, and whether the PR body's "Dependency justification" section explains it.
...
- **fail** if ANY of: governance files modified, critical or high security issue found, new deps without justification, unknown-source packages, secret detected, or hard invariants (rate-limit, auth, core config) per CLAUDE.md are touched.
